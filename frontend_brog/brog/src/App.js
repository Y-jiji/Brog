...
function UniViewer( { pageProxy, width, canvas_width } ) {
    // 获取viewport
...
function TestUniViewer() {
    ...
    const [ canvas_width, setCanvasWidth ] = useState(1000)
    ...
    return ( pageProxy ?
        <div>
            <button
                onClick={() => { setWidth( ( width ) => ( width + 50 ) );
                    setCanvasWidth((canvas_width + 50)); 
                }}
...
function MultiViewer( { pdfProxy, scale } ) {
    ...
    return (
        <table className={'abcd'} css={style} onScroll={onscroll}>
            {content}
        </table>
    )
}
